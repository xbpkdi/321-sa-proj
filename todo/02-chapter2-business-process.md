...
- [ ] 2.4 Business Process ใหม่ (To-Be) — มีไฟล์ `biz-flow-to-be.png` แต่ **มีปัญหาต้องแก้** (label swimlane พิมพ์ผิด "f") ดู [`use-case-diagram fix` companion: `biz-flow-fix-notes.md`](../02-chapter2-business-process/biz-flow-fix-notes.md)
- [ ] 2.5 คำอธิบาย Business Process ใหม่ — ยังไม่เขียน (ต้องระบุว่า process ใด manual/automatic และ process ใดแก้ปัญหาข้อใน 2.3)
- [ ] 2.6 Business Process ที่สัมพันธ์กับ Use Case — มีไฟล์ `biz-flow-rel-use-case.png` แต่ **มีปัญหาต้องแก้** (ชื่อ UC ไม่ตรง process, เลข UC ชนกัน) ดู [`02-chapter2-business-process/biz-flow-fix-notes.md`](../02-chapter2-business-process/biz-flow-fix-notes.md)
...
- [ ] 2.11 Use Case ที่ปรับปรุงแล้ว — มีไฟล์ `use-case.png` แต่ **ยังเป็น WIP** (20 UC: เจ้าของ 15, Delivery 2, Marketplace 2, Supplier 1) ต้องตรวจทาน/สรุปให้เป็นเวอร์ชันสุดท้าย
