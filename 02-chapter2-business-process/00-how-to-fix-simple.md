...
graph LR
  ...
  S --> S3(("3S) ตรวจสอบจำนวนสต๊อก<br/>เพียงพอไหม"))
  S --> S4(("4S) จับคู่เลข RSL<br/>กับ order_id"))
  S --> S5(("5S) พิมพ์ Label<br/>อัตโนมัติ"))
  S --> S6(("6S) คำนวณต้นทุน<br/>ต่อหน่วยอัตโนมัติ"))
  S --> S7(("7S) ส่ง Order ให้ Supplier<br/>อัตโนมัติ"))

  M4 -.->|ต้องอนุมัติก่อน ถึงจะทำ 7S| S7
...
**รวม 5 actor (M, S, R, D, P) — 16 UC**

**🔑 จุดสำคัญที่สุด (human-in-the-loop):** `4M) อนุมัติคำสั่งซื้อเพิ่มสต๊อก` ต้องเป็นของ **เจ้าของ/Admin** เท่านั้น — `7S` (ส่ง Order ให้ Supplier อัตโนมัติ) ทำได้ก็ต่อเมื่อ 4M อนุมัติแล้ว ห้ามให้ระบบข้ามขั้นนี้ไปเอง เพราะเป็นเรื่องเงิน (อาจารย์คอมเม้นไว้)

### วิธีทำ

1. ลบ `use-case.png` เวอร์ชันเก่า (20 UC เดิม) ทิ้ง — ไม่ตรงกับโครงสร้างใหม่นี้แล้ว
2. วาดใหม่ตาม 5 actor + 16 UC ข้างบน
3. เน้น `4M` กับ `7S` ให้เห็นชัดว่าต้องผ่านการอนุมัติก่อน (ใส่กรอบ/สี/comment กำกับ)
...
ทำหลังจากงานที่ 2 เสร็จแล้วเท่านั้น เพราะต้องใช้เลข UC เวอร์ชัน To-be ใหม่ (S1-S7, M1-M4, D1-D2, P1-P2, R1)
...
**แก้เป็น:** ให้ตรงกับ `7S`/`2D` คนละกล่องคนละความหมาย

```mermaid
graph TD
  D2["Delivery (2D) ส่งเลขติดตามกลับมา"] --> P9["กล่อง 9 = 7S:<br/>ระบบส่งเลขติดตามให้ลูกค้า (ผ่าน Rakuten)"]
  D2 --> P8["กล่อง 8:<br/>ระบบติดตามสถานะจัดส่งจนถึงลูกค้า"]
```
...
|---|---|---|
| "อัพเดท**สถานะการจัดส่ง**" | "Update **จำนวนสินค้า**ใน Marketplace" | เปลี่ยนป้ายให้ตรงกับ UC ที่ถูกต้องจริง (ไม่มี UC นี้ในชุดใหม่ ให้พิจารณาว่าเป็นส่วนหนึ่งของ `1S` หรือ `7S`) |
...
- [ ] มี UC ครบ 16 อัน (1M-4M, 1S-7S, 1R, 1D-2D, 1P-2P)
- [ ] `4M` (อนุมัติสั่งซื้อ) เป็นของเจ้าของ/Admin ไม่ใช่ระบบ
- [ ] `7S` ต้องเกิดหลัง `4M` อนุมัติแล้วเท่านั้น (มี note/เส้นกำกับ)
- [ ] ลบ use-case.png เวอร์ชัน 20 UC เดิมทิ้งแล้ว

**งานที่ 3 — biz-flow**
- [ ] กล่อง 8-9 ไม่ซ้ำข้อความกันแล้ว ตรงกับ `7S`/กล่องติดตามสถานะ
