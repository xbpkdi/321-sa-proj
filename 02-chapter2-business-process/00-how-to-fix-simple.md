...
graph LR
  ...
  S --> S6(("6S) ส่งเลขติดตามสินค้า<br/>ให้ลูกค้า"))
...
- `6S) ส่งเลขติดตามสินค้าให้ลูกค้า` — แยกออกจาก `9S` ชัดเจน (ก่อนหน้านี้สับสนกันอยู่ ทำให้ทั้งสอง UC มีความหมายเดียวกันโดยไม่ตั้งใจ)
...
```mermaid
graph TD
  D2["Delivery (2D) ส่งเลขติดตามกลับมา"] --> P9["กล่อง 9 = 6S:<br/>ระบบส่งเลขติดตามให้ลูกค้า (ผ่าน Rakuten)"]
  D2 --> P8["กล่อง 8 = 3S:<br/>ระบบติดตามสถานะจัดส่งจนถึงลูกค้า"]
```
...
- [ ] มี `6S` (ส่งเลขติดตามให้ลูกค้า) และ `7S` (ตัดสต๊อก) แล้ว — 2 อันนี้เคยตกหล่นในเวอร์ชันก่อนๆ
- [ ] ลบ use-case.png เวอร์ชัน 20 UC เดิมทิ้งแล้ว

**งานที่ 3 — biz-flow**
- [ ] กล่อง 8-9 ไม่ซ้ำข้อความกันแล้ว ตรงกับ `6S` (ส่งเลขติดตามลูกค้า) / `3S` (ติดตามสถานะ)
